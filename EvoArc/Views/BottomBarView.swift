...
struct BottomBarView: View {
    ...
    private let baseTabIndicatorSize: CGFloat = 28
    private let baseButtonSize: CGFloat = 44
    private let baseURLBarHeight: CGFloat = 44
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        .frame(maxHeight: isURLBarFocused ? baseURLBarHeight + suggestionsHeight : baseURLBarHeight)
                        .layoutPriority(1)
                    }
                    .keyboardAware(manager: keyboardManager, spacing: 8)
                }
            }

            // Suggestions overlay above the bottom bar (when shown)
            if isURLBarFocused && !urlEditingText.isEmpty && !suggestionManager.suggestions.isEmpty {
                ...
                .padding(.bottom, keyboardManager.keyboardHeight + UIScaleMetrics.maxDimension(baseURLBarHeight) + 8)
            }
        }
